...
export async function generateFinancialResponse(
  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      model: "gpt-4o", // Using gpt-4o which has better compatibility
      messages: [
        {
          role: "system",
        ...
        {
          role: "user",
          content: `${conversationContext}${accountContext}`
        }
      ],
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: `You are an expert financial analyst AI. Your task is to categorize financial transactions based on their descriptions.
...
Do not include any other text, explanations, or markdown fences in your response.`
        ...
        {
          role: "user",
          content: `Transactions to categorize:\n${JSON.stringify(transactions)}`
        }
      ],
...
`;
    ...
    const analysisResponse = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "user",
          content: dataAwareSystemInstruction
        }
      ],
      max_completion_tokens: 1500,
      temperature: 0.7
